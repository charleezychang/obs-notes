...
- track state to optimize rendering updates
- without signals, Angular runs on life cycle / change detection
- could be writable or read-only (computed signals)
- v16+
...
- `computed` - produces another signal based from other signals (automatically detects dependencies, dynamic: does not read signals inside false blocks), these are read-only, lazily evaluated (calculate only until the first time you read it), and memoized (cached)
- `effect` - should be rarely used, runs whenever one or more signal values change (dynamic), only used within an injection context (otherwise, able to pass `{injector: this.injector}` as a second parameter)
...
- accepts a clean up function as its first parameter to destroy long-running operations
```ts
effect((onCleanup) => {
...
- Can also be directly used on a signal
```ts
...
constructor() {
	...
	effect(() => {
	  console.log(`User: ${currentUser()}, the counter is ${untracked(counter)}`);
	});
}
```
...
- must be run in an injector context
...
```
- Opposite is to use `toObservable(this.mySignal())`, only the first emission is synchronous, meaning updating a signal's value multiple times, `toObservable` will only emit the value after the signal stabilizes
```ts
