...
- ==Providing it at the root level using `providedIn`==: creates a shared instance and able to inject in any class that asks for it. Enables Angular and JavaScript code to remove services that are unused from the bundle called *tree-shaking*
```ts
@Injectable({
  providedIn: 'root'
})
