- comments are omitted from the final build
- components can be self-closed
- `@` is a special character to add dynamic behavior (recent versions), use `&#64;` for a literal character
- ignores whitespace but can be configured not to
#### 1. ==Text Interpolation==
- denoted by double braces`{{ }}`
- objects and arrays are converted using the value's `toString` method, use `| json` to read them in the template
- this can be used in properties in attributes i.e. `[src]="{{ source }}`
```ts
export class AppComponent {
  myArray = [1, 2, 3]; // converted to 1, 2, 3
  myObject = { name: 'Angular', version: 12 }; // converted to [object Object]
}
```
...
```
#### 2. ==Property Binding==
- uses square brackets `[]` which can be used on native HTML element properties, component properties, or even directive properties
...
#### 3. ==Event Binding: Event listeners==
```html
...
```
#### ==Control flow== (previously `*ngIf`, `*ngFor`, `*ngSwitch`)
```html
@if (a > b) {
  {{a}} is greater than {{b}}
} @else if (b > a) {
...
- use `@Pipe` decorator which accepts metadata of `name` (used in the template that uses this pipe operator), `standalone`, and `pure` (false when you want a pipe to detect changes within arrays or objects)
...
- `ng-template` contents are NOT rendered on the page unless it is used with a structural directive specially with `ngTemplateOutlet`
- more for grouping sections to be rendered and reusing these sections
...
export class ComponentWithFragment {
  // When querying by name, you can use the `read` option to specify that you want to get the TemplateRef object associated with the element.
...
```
##### `ng-container`
- does not render a real element in the DOM but renders the content it encloses
